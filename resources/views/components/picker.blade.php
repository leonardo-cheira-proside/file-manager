...
<div x-data="{
     ...
     @file-manager-selected.window="if (open) { selected = $event.detail.paths || []; open = false; }"
    ...
    <div class="flex items-center gap-3 p-3 border border-dashed border-gray-300 rounded-xl bg-gray-50/50 flex-wrap">
        <button type="button" @click="open = true"
                class="px-4 py-2 bg-teal-600 text-white text-sm font-semibold rounded-lg hover:bg-teal-700 transition flex items-center gap-2 shrink-0">
        ...
        <template x-for="(path, i) in selected" :key="i">
            <div class="relative group flex items-center">
                ...
                <button type="button" @click.stop="selected.splice(i, 1)" title="@lang('file-manager::file-manager.remove')"
