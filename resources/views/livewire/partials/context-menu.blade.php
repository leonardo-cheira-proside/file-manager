...
<div x-show="menu.open" x-cloak @click.outside="menu.open = false"
    ...
    <div class="px-4 py-2 border-b mb-1 border-gray-100 font-bold text-teal-700 truncate text-xs"
         x-text="menu.files.length > 1 ? (menu.files.length + ' @lang('file-manager::file-manager.items_selected')') : (menu.file ? menu.file.name : '')"></div>
    ...
    @if ($this->inTrash)
        ...
        <button type="button" @click="$wire.deleteForever(targetPaths()); menu.open=false"
                class="w-full text-left px-4 py-2.5 text-red-600 hover:bg-teal-50 flex items-center gap-2">
    ...
    @else
            ...
            <div>
                <template x-if="menu.file.type === 'folder'">
                    <button type="button" @click="openModal({ action: 'add', type: 'folder', path: menu.file.path })"
                            class="w-full text-left px-4 py-2.5 hover:bg-teal-50 flex items-center gap-2">
                        <svg class="w-4 h-4 text-teal-600" fill="currentColor" viewBox="0 0 20 20"><path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"/></svg>
                ...
                <button type="button" @click="openModal({ action: 'rename', file: menu.file, text: fmStripExt(menu.file) })"
                        class="w-full text-left px-4 py-2.5 hover:bg-teal-50 flex items-center gap-2">
                    <svg class="w-4 h-4 text-teal-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
        ...
        <template x-if="everySelectedIs(['image','video','other'])">
            <button type="button" @click="download()"
                    class="w-full text-left px-4 py-2.5 hover:bg-teal-50 flex items-center gap-2">
                <svg class="w-4 h-4 text-teal-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
